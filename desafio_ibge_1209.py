...
class SidraAutomation:
    ...
    def _search_table(self, page, table):
        try:
            ...
            search_icon_locator = page.wait_for_selector(
                'a[title="Pesquisa Tabela"]', timeout=DEFAULT_TIMEOUT_MS
            )
            search_icon_locator.click()

            print("Digitando string no campo de pesquisa...")

            search_input_locator = page.wait_for_selector(
                '#sidra-pesquisa-lg input[placeholder="pesquisar"]',
            ...
            search_button_locator = page.wait_for_selector(
                '#sidra-pesquisa-lg button:has-text("OK")', timeout=DEFAULT_TIMEOUT_MS
            )
    ...
    def configure_table(self, page):
        ...
        try:
            ...
            for item_locator in all_list_items:
                ...
                toggle_button_locator = item_locator.locator(
                    ".nome-arvore .sidra-toggle"
                ).first

                # Item que queremos selecionar "Unidade da Federação"
                is_target_item = (
                    item_locator.text_content().strip().startswith(target_label)
                )

                # Check the current state using the parent div's class
                is_currently_checked = "checked" in item_locator.locator(
                    ".sidra-check"
    ...
    def download_csv(self, page):
        ...
        page.wait_for_selector(
            "#modal-downloads.in", state="visible", timeout=MODAL_TIMEOUT_MS
        )

        select_locator = page.wait_for_selector(
            '#modal-downloads select[name="formato-arquivo"]', timeout=MODAL_TIMEOUT_MS
        )
        ...
        download_button = page.wait_for_selector(
            "#opcao-downloads", timeout=DOWNLOAD_TIMEOUT_MS
        )
